...
pub trait Buffer<T>:
  AsMut<[T]> + cl_aux::Clear + cl_aux::Extend<T, Error = cl_aux::Error, Output = ()>
{
}

impl<T, U> Buffer<T> for U where
  U: AsMut<[T]> + cl_aux::Clear + cl_aux::Extend<T, Error = cl_aux::Error, Output = ()>
{
}

macro_rules! create_byte_buffer {
  ($($bounds:tt)*) => {
    /// Specialized [Buffer] trait for opaque bytes.
