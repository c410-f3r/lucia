        ...
        if let Some(n) = inferred_additional_tries.checked_sub(1) {
          ...
          for _ in 0..n {
            let $local_blockhash = {
              ...
              let (rmw, trans) = pair_mut.parts_mut();
              let req = rmw.get_latest_blockhash(None);
              let res = trans.send_retrieve_and_decode_one(rmw, &req, ()).await?;
...
macro_rules! _create_blockchain_constants {
  (
    ...
    address_hash_str: $address_hash_str:ident = $_2:literal,
    block_hash: $block_hash:ident = $_3:literal,
    block_hash_str: $block_hash_str:ident = $_4:literal,
    signature_hash: $signature_hash:ident = $_5:literal,
    signature_hash_str: $signature_hash_str:ident = $_6:literal,
    transaction_hash: $transaction_hash:ident = $_7:literal,
    transaction_hash_str: $transaction_hash_str:ident = $_8:literal
  ) => {
    /// Address hash as bytes
    pub(crate) type $address_hash = [u8; $_1];
    /// Address hash as an encoded string
    pub(crate) type $address_hash_str = ::arrayvec::ArrayString<$_2>;

    /// Block hash as bytes
    pub(crate) type $block_hash = [u8; $_3];
    /// Block hash as an encoded string
    pub(crate) type $block_hash_str = ::arrayvec::ArrayString<$_4>;

    /// Signature hash as bytes
    pub(crate) type $signature_hash = ::cl_aux::ArrayWrapper<u8, $_5>;
    /// Signature hash as an encoded string
    pub(crate) type $signature_hash_str = ::arrayvec::ArrayString<$_6>;

    /// Transaction hash as bytes
    pub(crate) type $transaction_hash = ::cl_aux::ArrayWrapper<u8, $_7>;
    /// Transaction hash as an encoded string
    pub(crate) type $transaction_hash_str = ::arrayvec::ArrayString<$_8>;
  };
}
...
macro_rules! _create_endpoint {
  (
    ...
    impl<
      ...
      $( const $build_const: $build_const_ty, )*
      $( $build_ty: $(for<$build_path_lt>)? $($build_path +)*, )*
    > lucia::req_res::RequestResponse<RESP> for $struct_name<
      $( $build_lt, )*
      $( $build_const, )*
      $( $build_ty, )*
    >
  ...
  ($executor:ident, $test:ident, $pair:expr, $parts_cb:expr, $rslt_cb:expr $(, $(#[$attrs:meta])+)?) => {
    ...
    async fn $test() {
      fn parts_cb_infer<'pair, A, CP, DRSR, O, T>(
        rmw: &'pair mut crate::misc::RequestManagerWrapper<A, CP, DRSR>,
        trans: &'pair mut T,
      ...
      let mut pair = $pair;
      let (rmw, trans) = pair.parts_mut();
      let rslt = parts_cb_infer(rmw, trans, $parts_cb).await;
      rslt_cb_infer(rmw, trans, rslt, $rslt_cb).await;
    }
  };
}
...
macro_rules! _create_tokio_tungstenite_test {
  ($cp_drsr:expr, $sub:ident, ($($unsub:ident),+), $cb:expr $(, $(#[$attrs:meta])+)?) => {
    _create_generic_test! {
      ...
      $cb,
      |rmw, trans, subs| async move {
        let mut iter = subs.into_iter();
        let ids = [$( rmw.$unsub(iter.next().unwrap()), )+];
        let _ = trans.send(rmw, &ids, ()).await.unwrap();
      }
      $(, $(#[$attrs])+)?
    }
  };
