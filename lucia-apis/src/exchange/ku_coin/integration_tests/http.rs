use crate::exchange::ku_coin::{
  ...
  PaymentAccountType, V1BulletParams,
};
use lucia::{misc::GenericTime, network::transport::Transport};
...
_create_http_test!(cred_test(), http_test(), v2_inner_transfers, |pkgs_aux, trans| async {
  let _res = trans
    .send_retrieve_and_decode_contained(
      &mut pkgs_aux
        .v2_inner_transfer()
        .data(
          &GenericTime::now().unwrap().timestamp().unwrap().as_nanos(),
          "USDT",
          PaymentAccountType::Trade,
          crate::exchange::ku_coin::ReceivingAccountType::Main,
          "0.1",
          None,
          None,
        )
