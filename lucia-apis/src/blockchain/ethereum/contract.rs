  ...
  T: Transport<Ethereum, CP, DRSR>,
{
  abi: ethabi::Contract,
  address: Address,
  ethereum: Pair<RequestManagerWrapper<Ethereum, CP, DRSR>, T>,
}
  ...
  T: Send + Transport<Ethereum, CP, DRSR>,
{
    ...
    JsonRpcResponse<Option<H256>>: for<'de> Deserialize<'de, DRSR>,
  {
    ...
    let (rmw, trans) = self.ethereum.parts_mut();
    let req = rmw.eth_send_transaction(&tr);
    Ok(trans.send_retrieve_and_decode_one(rmw, &req, ()).await?.result)
  }
    ...
    JsonRpcResponse<U256>: for<'de> Deserialize<'de, DRSR>,
  {
    ...
    let (rmw, trans) = self.ethereum.parts_mut();
    let req = rmw.eth_estimate_gas(None, &call_request);
    Ok(trans.send_retrieve_and_decode_one(rmw, &req, ()).await?.result)
  }
    ...
    JsonRpcResponse<Option<Vec<Log>>>: for<'de> Deserialize<'de, DRSR>,
  {
    ...
    let (rmw, trans) = self.ethereum.parts_mut();
    let req = rmw.eth_get_logs(&filter);
    let logs = if let Some(el) = trans.send_retrieve_and_decode_one(rmw, &req, ()).await?.result {
      el
    } else {
      return Ok(Vec::new());
    };
  ...
  where
    ...
    for<'cr> JsonRpcRequest<EthCallReq<'cr>>: Request<
        ...
        T::ResponseParams,
        Error = crate::Error,
        ProcessedResponse = ProcessedJsonRpcResponse<Option<crate::blockchain::ethereum::Bytes>>,
        RawResponse = JsonRpcResponse<Option<crate::blockchain::ethereum::Bytes>>,
      > + Serialize<DRSR>,
    JsonRpcResponse<Option<crate::blockchain::ethereum::Bytes>>: for<'de> Deserialize<'de, DRSR>,
  {
    ...
    let (rmw, trans) = self.ethereum.parts_mut();
    let req = rmw.eth_call(block_id, &call_request);
    trans
      .send_retrieve_and_decode_one(rmw, &req, ())
      .await?
      .result
...
mod tests {
  ...
  use ethereum_types::{H256, U256};
  use lucia::{
    data_formats::{JsonRpcRequest, JsonRpcResponse},
    dnsn::SerdeJson,
  ...
  fn contract(trans: &mut Test<str>) -> Contract<CommonParams<(), ()>, SerdeJson, &mut Test<str>> {
    let pair = Pair::new(
      RequestManagerWrapper::new(Ethereum, CommonParams::default(), SerdeJson::default()),
      trans,
    );
