...
//! let _res = trans.send_and_retrieve(&mut rmw, &req, ()).await?;
//! # Ok(()) }
//! ```
...
_create_blockchain_constants!(
  ...
  transaction_hash: _SolanaTransactionHash = 64,
...
impl Solana {
  ...
  pub async fn confirm_transaction<'tx_hash, CP, DRSR, T>(
    ...
    tx_hash: &'tx_hash str,
    rmw: &mut RequestManagerWrapper<Self, CP, DRSR>,
    trans: &mut T,
  ) -> crate::Result<bool>
  where
    CP: Send,
      ...
      for<'de> Deserialize<'de, DRSR>,
  {
    macro_rules! call {
      () => {{
        let req = rmw.get_signature_statuses(None, [tx_hash].into());
        if let &[Some(GetSignatureStatusesRes {
          confirmation_status: Commitment::Finalized, ..
        }), ..] = &*trans.send_retrieve_and_decode_one(rmw, &req, ()).await?.result.value
        {
          true
        } else {
          false
