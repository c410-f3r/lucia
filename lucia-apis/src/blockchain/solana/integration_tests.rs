...
_create_http_test!(http(), get_account_info, |rmw, trans| async {
  let req = rmw.get_account_info(
    TO_NORMAL_ACCOUNT,
  ...
  let _ = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result.value.unwrap();
});

_create_http_test!(http(), get_balance, |rmw, trans| async {
  let req = rmw.get_balance(TO_NORMAL_ACCOUNT, None);
  let _ = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap();
});

_create_http_test!(http(), get_block_height, |rmw, trans| async {
  let req = rmw.get_block_height(None, None);
  let _ = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap();
});

#[cfg(feature = "solana-program")]
_create_http_test!(http(), get_fee_for_message, |rmw, trans| async {
  let mut buffer = Vec::new();
  let from_keypair = Keypair::from_bytes(&alice_keypair()[..]).unwrap();
  let from_public_key = from_keypair.public.to_bytes();
  let req = rmw.get_latest_blockhash(None);
  let blockhash =
    trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result.value.blockhash;
  let message = transfer_message(blockhash, from_public_key);
  let req = rmw.get_fee_for_message(&mut buffer, &message, None).unwrap();
  assert_eq!(
    trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result.value.unwrap(),
    5000
  );
});

_create_http_test!(http(), get_minimum_balance_for_rent_exemption, |rmw, trans| async {
  let req = rmw.get_minimum_balance_for_rent_exemption(100, None);
  let _ = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap();
});

_create_http_test!(http(), get_multiple_accounts, |rmw, trans| async {
  let req = rmw.get_multiple_accounts(&[TO_NORMAL_ACCOUNT, TO_SOL_TOKEN_ACCOUNT], None);
  let _ = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap();
});

_create_http_test!(http(), get_program_accounts, |rmw, trans| async {
  let req = rmw.get_program_accounts(
    TOKEN_PROGRAM,
  ...
  assert_eq!(trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result.len(), 1);
});

_create_http_test!(http(), get_slot, |rmw, trans| async {
  let req = rmw.get_slot(None);
  let _ = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap();
});

_create_http_test!(http(), get_token_account_balance, |rmw, trans| async {
  let req = rmw.get_token_account_balance(TO_SOL_TOKEN_ACCOUNT, None);
  let _ = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap();
});

_create_http_test!(http(), get_token_accounts_by_owner, |rmw, trans| async {
  let req = rmw.get_token_accounts_by_owner::<16, _>(
    TO_NORMAL_ACCOUNT,
  ...
  let res = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap();
  let _ = res.result.value[0].pubkey;
});

_create_http_test!(http(), get_version, |rmw, trans| async {
  let req = rmw.get_version();
  let res = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap();
...
_create_http_test!(
  ...
  |rmw, trans| async {
    ...
    buffer.clear();
    let req = rmw.send_transaction(&mut buffer, None, &tx).unwrap();
    let tx_hash = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result;
    assert!(Solana::confirm_transaction(None, &tx_hash, rmw, trans).await.unwrap());

    let req = rmw.get_transaction(
      tx_hash.as_str(),
    ...
    let _ = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result.transaction;
    ...
    let tx = trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result.transaction;
    if !matches!(
...
_create_http_test!(http(), http_reqs_with_array, |rmw, trans| async {
  let first = rmw.get_balance(TO_NORMAL_ACCOUNT, None);
  let second = rmw.get_balance(TO_NORMAL_ACCOUNT, None);
  let mut buffer = Vec::new();
  let _ = trans.send_retrieve_and_decode_many(&mut buffer, rmw, &[first, second], ()).await.unwrap();
});
...
_create_tokio_tungstenite_test!(
  ...
  (account_unsubscribe),
  |rmw, trans| async {
    let req = rmw.account_subscribe(
      TO_NORMAL_ACCOUNT,
      Some(CommitmentOptEncoding { commitment: None, encoding: AccountEncoding::JsonParsed }),
    );
    [trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result]
  }
);

_create_tokio_tungstenite_test!(ws(), root_subscribe, (root_unsubscribe), |rmw, trans| async {
  let req = rmw.root_subscribe();
  [trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result]
});

_create_tokio_tungstenite_test!(ws(), slot_subscribe, (slot_unsubscribe), |rmw, trans| async {
  let req = rmw.slot_subscribe();
  [trans.send_retrieve_and_decode_one(rmw, &req, ()).await.unwrap().result]
});
...
_create_tokio_tungstenite_test!(
  ...
  |rmw, trans| async {
    let first = rmw.account_subscribe(
      TO_NORMAL_ACCOUNT,
      Some(CommitmentOptEncoding { commitment: None, encoding: AccountEncoding::JsonParsed }),
    );
    let second = rmw.account_subscribe(
      TO_NORMAL_ACCOUNT,
      Some(CommitmentOptEncoding { commitment: None, encoding: AccountEncoding::JsonParsed }),
    );
    let mut buffer = Vec::new();
    trans.send_retrieve_and_decode_many(&mut buffer, rmw, &[first, second], ()).await.unwrap();
