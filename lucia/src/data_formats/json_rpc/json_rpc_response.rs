use crate::{
  data_formats::ProcessedJsonRpcResponse,
  dnsn::{Deserialize, Serialize},
  misc::ByteBuffer,
  Id, MAX_JSON_RPC_METHOD_LEN,
};
use arrayvec::ArrayString;

/// Replied from a given [crate::data_formats::JsonRpcRequest].
///
...
impl<R> JsonRpcResponse<R> {
  /// Transfers all data from [JsonRpcResponse] to [ProcessedJsonRpcResponse] with a custom
  /// callback logic for `result`.
  #[inline]
  pub fn into_processed<PRR>(
    self,
...
#[cfg(feature = "serde")]
mod serde {
  use crate::data_formats::JsonRpcResponse;
  use alloc::boxed::Box;
          ...
          while let Some(key) = map.next_key()? {
            match key {
              Field::Error => {
                ...
                error = Some(map.next_value::<Box<crate::data_formats::JsonRpcResponseError>>()?);
              }
              Field::Id => {
                if id.is_some() {
...
mod serde_json {
  ...
  use crate::{data_formats::JsonRpcResponse, dnsn::SerdeJson, misc::seq_visitor::_SeqVisitor};
