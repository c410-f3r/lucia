use crate::{
  ...
  req_res::{Request, RequestParamsModifier, RequestResponse},
  Id,
};
use alloc::vec::Vec;
...
where
  ...
  REQ: Borrow<Id> + PartialOrd + Request<CP, REQP, RESP>,
...
impl<BU, CP, DRSR, REQ, REQP, RESP> Requests<BU, CP, DRSR, REQP, RESP> for [REQ]
where
  BU: Buffer<REQ::ProcessedResponse>,
  REQ: Borrow<Id> + PartialOrd + Request<CP, REQP, RESP>,
  REQ::ProcessedResponse: Borrow<Id> + Ord,
...
#[inline]
fn push_responses_from_reqs_slice<BU, DRSR, REQ, RESP>(
  buffer: &mut BU,
  bytes: &[u8],
  drsr: &mut DRSR,
  resp: &RESP,
...
where
  BU: Buffer<REQ::ProcessedResponse>,
  REQ: Borrow<Id> + PartialOrd + RequestResponse<RESP>,
  REQ::ProcessedResponse: Borrow<Id> + Ord,
  REQ::RawResponse: for<'de> Deserialize<'de, DRSR>,
{
  if slice.is_empty() {
...
fn search_slice<REQ, RESP>(idx: usize, res_id: Id, reqs: &[REQ]) -> crate::Result<(&REQ, usize)>
where
  REQ: Borrow<usize> + RequestResponse<RESP>,
{
